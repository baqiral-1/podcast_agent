...
def _build_synthesis_primitives(
    *,
...
def _build_strategy(
    *,
    title: str = "Episode",
    series_explanation_registry: list[dict[str, Any]] | None = None,
) -> NarrativeStrategy:
...
def test_resume_from_synthesis_primitives_reruns_synthesis_and_downstream(
    ...
    class FakeOrchestrator:
        def __init__(self, settings: Any) -> None:
            ...
            self.run_logger = SimpleNamespace(
                log=lambda event, **kwargs: calls.setdefault("run_logger_events", []).append(
                    (event, kwargs)
                )
            )
        ...
        async def _map_synthesis(
            ...
            *,
            ...
            primitives = _build_synthesis_primitives(quality_score=0.1)
    ...
    asyncio.run(resume_script._resume_from_synthesis_primitives("run_1"))
    ...
    assert calls["run_logger_events"] == [
        (
...
def test_resume_from_synthesis_primitives_rejects_axis_mismatch(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    project_dir = _build_project_dir(tmp_path)
...
def test_resume_from_synthesis_primitives_requires_actor_metadata(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    project_dir = _build_project_dir(tmp_path)
...
def test_resume_from_synthesis_primitives_fails_when_upstream_artifact_changes(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    project_dir = _build_project_dir(tmp_path)
...
def test_resume_from_synthesis_primitives_uses_stage_label_in_resume_failures(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    ...
    with pytest.raises(
        RuntimeError,
        match="Resume failed from synthesis primitives onward: episode 1: boom",
    ):
        asyncio.run(resume_script._resume_from_synthesis_primitives("run_1"))
