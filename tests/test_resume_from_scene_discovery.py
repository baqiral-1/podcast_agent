...
def _build_event_primitive(*, primitive_id: str = "primitive_1") -> EventPrimitive:
...
def _build_substrate_primitives(
    *, primitive_id: str = "primitive_1"
) -> SynthesisPrimitivesArtifact:
    return SynthesisPrimitivesArtifact(
        project_id="run_1",
        primitives=[_build_event_primitive(primitive_id=primitive_id)],
    )


def _build_tagged_primitives(
    *, primitive_id: str = "primitive_1"
) -> PrimitiveFunctionTaggingArtifact:
    return PrimitiveFunctionTaggingArtifact(
        project_id="run_1",
        primitives=[_build_event_primitive(primitive_id=primitive_id)],
    )
            ...
            "candidates": [
                {
                    ...
                    "scene_sketch": "A staged turning point becomes legible.",
                    "scene_jobs": ["opening"],
                    "anchor_image": "A file on a desk.",
...
def _build_project_dir(
    tmp_path: Path,
    *,
    substrate_primitive_id: str = "primitive_1",
    tagged_primitive_id: str = "primitive_1",
) -> Path:
    ...
    _write_json(
        ...
        _build_substrate_primitives(primitive_id=substrate_primitive_id),
    )
    _write_json(
        project_dir / "tagged_primitives.json",
        _build_tagged_primitives(primitive_id=tagged_primitive_id),
    )
    _write_json(
        project_dir / "actor_metadata_metrics.json",
        {"stage_metrics": {"substrate_function_tagging": {"unknown_actor_ids": 0}}},
    )
    return project_dir


def test_resume_from_scene_discovery_parse_args(
    ...
    monkeypatch.setattr(
        ...
        "argv",
        [
            "resume_from_scene_discovery.py",
            "--project-id",
            "run_42",
        ],
    )
...
def test_resume_from_scene_discovery_main_runs_async_entrypoint(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    calls: dict[str, Any] = {}
...
def test_resume_from_scene_discovery_reruns_from_scene_discovery_and_downstream(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    ...
    class FakeOrchestrator:
        ...
        async def _discover_scenes(
            ...
            *,
            ...
            calls["scene_discovery_config"] = project.config
            calls["scene_discovery_input"] = synthesis_map
            calls["scene_discovery_actor_metadata"] = actor_metadata
        ...
        async def _choose_narrative_strategy(
            ...
            **kwargs: Any,
            ...
            calls["narrative_strategy_actor_metadata"] = actor_metadata
    ...
    asyncio.run(resume_script._resume_from_scene_discovery("run_1"))
    ...
    assert calls["scene_discovery_config"].skip_grounding is True
...
def test_resume_from_scene_discovery_rejects_axis_mismatch(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    project_dir = _build_project_dir(tmp_path)
...
def test_resume_from_scene_discovery_requires_actor_metadata(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    project_dir = _build_project_dir(tmp_path)
...
def test_resume_from_scene_discovery_requires_substrate_primitives(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    project_dir = _build_project_dir(tmp_path)
...
def test_resume_from_scene_discovery_requires_tagged_primitives(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    project_dir = _build_project_dir(tmp_path)
...
def test_resume_from_scene_discovery_rejects_primitive_id_mismatch(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _build_project_dir(
        tmp_path,
        substrate_primitive_id="primitive_1",
        tagged_primitive_id="primitive_2",
    )
    ...
    with pytest.raises(RuntimeError, match="disagree on primitive ids"):
        asyncio.run(resume_script._resume_from_scene_discovery("run_1"))


def test_resume_from_scene_discovery_fails_when_upstream_artifact_changes(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    project_dir = _build_project_dir(tmp_path)
    ...
    class FakeOrchestrator:
        ...
        async def _discover_scenes(
            ...
            *,
            ...
            _write_json(
                project_dir / "tagged_primitives.json",
                _build_tagged_primitives(primitive_id="changed_primitive"),
            )
            return _build_scene_discovery()
    ...
    with pytest.raises(
        RuntimeError,
        match="Strict integrity check failed: tagged_primitives.json changed during resume",
    ):
        asyncio.run(resume_script._resume_from_scene_discovery("run_1"))
...
def test_resume_from_scene_discovery_uses_stage_label_in_resume_failures(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    ...
    with pytest.raises(
        RuntimeError,
        match="Resume failed from scene discovery onward: episode 1: boom",
    ):
        asyncio.run(resume_script._resume_from_scene_discovery("run_1"))
