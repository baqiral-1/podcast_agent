...
async def _retry_single_episode_from_writing(
    project_id: str,
    episode_number: int,
) -> None:
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Retry writing, style audit, spoken delivery, and render manifest "
            "generation for one episode in an existing run using persisted "
            "planning artifacts."
        )
    )
    parser.add_argument(
