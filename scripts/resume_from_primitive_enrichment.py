...
def _load_persisted_architectures(project_dir: Path) -> list[EpisodeArchitecture]:
    architecture_path = project_dir / "episode_architectures.json"
...
async def _resume_from_primitive_enrichment(project_id: str) -> None:
    settings = Settings()
    ...
    try:
        ...
        _, architecture_actor_metrics = await orchestrator._build_episode_architectures(
            project=project,
        ...
        ep_tasks = [
            orchestrator._produce_episode(
                ...
                next(
                    item
                    for item in persisted_architectures
        ...
        if production_errors or audio_errors:
            ...
            raise RuntimeError(
                "Resume failed from primitive enrichment onward: " + "; ".join(details)
            )
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Resume an existing run from primitive enrichment onward using existing "
            "synthesis primitives, narrative strategy, and actor artifacts with strict "
            "integrity checks."
        )
    )
    parser.add_argument(
...
def main() -> None:
    args = _parse_args()
    asyncio.run(_resume_from_primitive_enrichment(args.project_id))
