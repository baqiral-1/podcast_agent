...
class NarrativeStrategyAgent(Agent):
    ...
    instructions = (
        ...
        "- thread arc types\n"
        "- thematic cohesion across axes\n"
        "- narrative potential (progression, contrast, and payoff)\n"
        ...
        "Target 8-10 insights per episode while keeping each episode coherent: usually 3-4 axes.\n"
        "If constraints conflict, prioritize full coverage of SynthesisInsights with "
        "podcast_potential > 0.5, even if an episode needs more than 11 insights.\n\n"
        "Every episode must have a single driving_question. Episode 1 may state the global "
        "series question directly. Episodes 2+ must ask a narrower causal or interpretive question "
        "that advances the argument using that episode's assigned material; do not simply restate "
        "the Episode 1 question.\n\n"
        "For each episode, infer 4-5 episode_inquiries from the assigned axes' guiding questions. "
        "Each inquiry must include axis_id and question, and axis_id must belong to that episode's "
        "assigned axes.\n\n"
        ...
        "Also choose a recommended_episode_count between 7 and 9 based on thematic density, "
        ...
        "  - episode_inquiries: array of 4-5 objects, each with axis_id and question\n"
        "  - payoff_shape: string\n"
        "- recommended_episode_count: integer between 7 and 9\n"
        ...
        "- Prefer assigning only the minimum IDs needed to sustain each episode's argument.\n\n"
