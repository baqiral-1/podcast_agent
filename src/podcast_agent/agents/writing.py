...
class WritingAgent(Agent):
    ...
    instructions = (
        ...
        "Requirements:\n"
        "- Follow the plan hierarchy in this order: narrative spine first, scene_cards second, beats within each scene third\n"
        "- Fully realize one scene before moving to the next unless a beat explicitly pivots threads\n"
        "- EpisodePlan field contract:\n"
        "  - Required writing constraints: treat plan.narrative_spine as binding structure; "
        "treat plan.scene_cards as the primary composition units; use plan.anchor_scene_ids as "
        "load-bearing scenes that deserve fuller dramatization and cleaner scene openings/exits; "
        "treat plan.driving_question as the episode's primary rhetorical anchor; preserve "
        "plan.unresolved_questions as live tensions when the episode leaves matters open; honor "
        "plan.payoff_shape in the ending movement; "
        "do not replace the episode's question with a broader series-level fallback except for "
        ...
        "plan.target_word_count with beat estimated_duration_seconds as pacing guidance\n"
        ...
        "- Use payload.beat_word_targets[*].target_words as beat-level pacing targets when beat_id is present\n"
        ...
        "- Use plan.synthesis_context to sharpen arc, transitions, disagreement framing, and unresolved questions\n"
        "- Use each scene card's entry_image, timeframe, location, actors, and exit_turn to shape scene openings and landings\n"
        "- Keep scenes in plan.scene_cards order and keep beats in order within each scene\n"
        ...
        "- scene_id: which scene this segment belongs to\n"
